...
void remove_newlineOrSpacesAtTheEnd(
    char *str) // From internet, added space deletion manually
{
...
void CreateAccount(struct account **accountList, int *count) {
  *accountList = (struct account *)realloc(
      *accountList, (*count + 1) * sizeof(struct account));
  if (*accountList == NULL) {
...
size_t FindAccountLoc(const struct account *accs, const int count,
  ...
  return count +
         999; // Random number just do its so wrong we assume its not found.
}

// 1 Mode = deposit
// 2 mode = withdraw
void HandleCash(struct account *accs, const int count, const char *nameToFind,
                const int mode) {
...
void RunBank() {
  ...
  char *nameToDisplay = NULL;
  char *nameToProcess = NULL;
  char *nameToFind = NULL;
  while (1) {
